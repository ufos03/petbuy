...
class Shopic_Widget_Layered_Nav extends WC_Widget {

    /**
     * Constructor.
     */
    public function __construct() {
        $this->widget_cssclass    = 'woocommerce shopic_widget_layered_nav woocommerce-widget-layered-nav';
        $this->widget_description = esc_html__('Display a list of attributes to filter products in your store.(Shopic custom)', 'shopic');
        $this->widget_id          = 'shopic_woocommerce_layered_nav';
        $this->widget_name        = esc_html__('Shopic Filter Products by Attribute', 'shopic');
        parent::__construct();
    }
    ...
    public function init_settings() {
        ...
        $this->settings = array(
            'title'      => array(
                'type'  => 'text',
                'std'   => esc_html__('Filter by', 'shopic'),
                'label' => esc_html__('Title', 'shopic'),
            ),
            'attribute'  => array(
                'type'    => 'select',
                'std'     => '',
                'label'   => esc_html__('Attribute', 'shopic'),
                'options' => $attribute_array,
            ),
            'query_type' => array(
                'type'    => 'select',
                'std'     => 'and',
                'label'   => esc_html__('Query type', 'shopic'),
                'options' => array(
                    'and' => esc_html__('AND', 'shopic'),
                    'or'  => esc_html__('OR', 'shopic'),
                ),
            ),
        );
    }
    ...
    protected function layered_nav_list($terms, $taxonomy, $query_type) {
        ...
        foreach ($terms as $term) {
            ...
            if ($count > 0 || $option_is_set) {
                ...
                $term_html = '<a rel="nofollow" class="shopic-button-type" href="' . $link . '">' . esc_html($term->name) . '</a>';
                    ...
                    if (isset($fields[0]["type"])) {
                        ...
                        switch ($fields[0]["type"]) {
                            case 'color':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="shopic-color-type" href="' . esc_url($link) . '">';
                            ...
                            case 'image':
                                ...
                                $term_html = '';
                                $term_html .= '<a rel="nofollow" class="shopic-image-type" href="' . esc_url($link) . '">';
