 ...
 * @extends WC_Widget
 */
class Shopic_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'shopic');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'shopic');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'shopic'),
				'label' => esc_html__('Title', 'shopic'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'shopic'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'shopic'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'shopic'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'shopic'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'shopic'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'shopic'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'shopic'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Shopic_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'shopic');
			...
			echo '<ul class="product-brands" id="shopic-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Shopic_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'shopic'),
		'singular_name'              => esc_html__('Brands', 'shopic'),
		'menu_name'                  => esc_html__('Brands', 'shopic'),
		'all_items'                  => esc_html__('All Brands', 'shopic'),
		'parent_item'                => esc_html__('Parent Brand', 'shopic'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'shopic'),
		'new_item_name'              => esc_html__('New Brand Name', 'shopic'),
		'add_new_item'               => esc_html__('Add New Brands', 'shopic'),
		'edit_item'                  => esc_html__('Edit Brand', 'shopic'),
		'update_item'                => esc_html__('Update Brand', 'shopic'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'shopic'),
		'search_items'               => esc_html__('Search Brands', 'shopic'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'shopic'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'shopic'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
