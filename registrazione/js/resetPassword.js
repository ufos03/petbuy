...
function call_reset_passoword_api()
{
    jQuery(".forward").on("click", function() {

        const urlParams = new URLSearchParams(location.search);
        const params = {token : urlParams.get("payload"), psw : jQuery("#pass").val()};
        console.log(params)

        jQuery.ajax({
            type: "POST",
            contentType: "application/json; charset=UTF-8",
            url: "https://petbuy-local.ns0.it:8080/wp-json/api/v1/update_password",
            data: JSON.stringify(params),
            success: function (response) {
                window.location.assign("https://petbuy-local.ns0.it:8080/login/")
            },
